...
def get_model_complexity_info(model, input_res, print_per_layer_stat=True, as_strings=True, channel=3, flop_units=None,
                              param_units=None):
    ...
    if print_per_layer_stat:
        print_model_with_flops(flops_model, flops_count, params_count, flops_units=flop_units, param_units=param_units)

    flops_model.stop_flops_count()

    if as_strings:
        return flops_to_string(flops_count, flop_units), params_to_string(params_count, param_units)
    ...
    def add_flops_counter_hook_function(module):
        if is_supported_instance(module):
            if hasattr(module, '__flops_handle__'):
                return
            if type(module).__name__ in CUSTOM_MODULES_MAPPING:
                handle = module.register_forward_hook(
                    CUSTOM_MODULES_MAPPING[type(module).__name__])
            else:
...
def is_supported_instance(module):
    if type(module) in MODULES_MAPPING or str(type(module).__name__) in CUSTOM_MODULES_MAPPING:
        return True
    return False
...
def matmul_flops_counter_hook(matmul_module, input, output):
    input0, input1 = input
    matmul_module.__flops__ += int(np.prod(input0.shape) * input1.shape[-1])
    # matmul_module.__flops__ += 100000000
...
CUSTOM_MODULES_MAPPING = {
    # matrix matmul
    "Matmul": matmul_flops_counter_hook
}
...
if __name__ == "__main__":
    ...
    from geoseg.models.SFFNet import SFFNet
    from geoseg.models.RS3Mamba import RS3Mamba
    
    from geoseg.models.our import UNetFormer
    
    
    device=torch.device("cpu")
    

    shape = torch.rand(1, 3, 1024, 1024)
    shape = shape.to(device)
    
    net = UNetFormer(num_classes=6).to(device)
    # net = ABCNet(3,6).to(device)
    # net = SFFNet(num_classes=6).to(device)
    # net = RS3Mamba(num_classes=6).cuda()
    
    # net = UNetFormer(num_classes=6).to(device)


    # 将 shape 转换为元组，提取后两个维度
    input_shape = (shape.shape[2], shape.shape[3])


    flops_count, params_count = get_model_complexity_info(net, input_shape)

    print("flops_count: " + str(flops_count))
    print("params_count: " + str(params_count))
