...
@register_model
def cdfformer_s18(features_only = False , pretrained=False, **kwargs):
    # default_cfg, 9个dict， num_classes 100, input_size [3 224 224], classier 'head'
...
def cdfformer_m36(pretrained=False, **kwargs):
    ...
    # depths=[3, 12, 18, 3],表示模型每个阶段的MetaFormerBlock数量，模型有4个阶段，第一阶段3个
    # dims=[96, 192, 384, 576],表示每个阶段的特征维度，第一阶段维度是96
    # token_mixers，指定每个阶段使用的token_mixer， 前两个用 SepConv，后两个用 DynamicFilter

    # https://github.com/okojoalg/dfformer/releases/download/weights/cdfformer_m36.pth
    # default_cfg, 9个dict， num_classes 100, input_size [3 224 224], classier 'head'
